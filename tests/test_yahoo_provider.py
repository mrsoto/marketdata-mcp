...
def test_yahoo_retries_invalid_ohlc_then_returns_valid_series() -> None:
    ...
    with patch("market_mcp.providers.yahoo.yf.Ticker", return_value=ticker), patch("market_mcp.providers.yahoo.time.sleep") as pause:
...
def test_yahoo_reports_missing_after_persistent_invalid_ohlc() -> None:
    ...
    with patch("market_mcp.providers.yahoo.yf.Ticker", return_value=ticker), patch("market_mcp.providers.yahoo.time.sleep"):
...
def test_yahoo_ignores_incomplete_ohlc_rows() -> None:
    ...
    with (
        patch("market_mcp.providers.yahoo.yf.Ticker", return_value=ticker),
        patch("market_mcp.providers.yahoo.time.sleep") as pause,
    ):
